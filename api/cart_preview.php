...
.dropdown-cart .close-btn, .dropdown-mycourses .close-btn { position:absolute; top:5px; right:10px; font-size:20px; cursor:pointer; }
...
$(document).ready(function(){
    ...
    function toggleDropdown(dropdown){
        if($(window).width() < 768){
            ...
            $('.dropdown-cart, .dropdown-mycourses').removeClass('active');
    ...
    $('.nav-icons > div').each(function(){
        let dropdown = $(this).find('.dropdown-cart, .dropdown-mycourses');
        ...
        // Close button
        dropdown.find('.close-btn').click(function(){
                ...
                resp.items.forEach(item=>{
                    html += `<div class="item">
                        ...
                        <div class="item-info">
                            ...
                            <div class="price">NPR ${Number(item.price).toLocaleString()}</div>
                        </div>
                        <a href="<?= BASE_URL ?>/payments/cart.php?remove=${item.id}" class="btn btn-sm btn-danger">Remove</a>
                    </div>`;
                });
                html += `<div class="text-center mt-2">
                            <a href="<?= BASE_URL ?>/payments/cart.php" class="btn btn-sm btn-success">Go to Cart</a>
                         </div>`;
            }
            $('.dropdown-cart').html('<span class="close-btn">&times;</span>'+html);
            $('.dropdown-cart .close-btn').click(()=>{ $('.dropdown-cart').removeClass('active'); overlay.removeClass('active'); });
        }
    });
    ...
    $.getJSON('<?= BASE_URL ?>/api/my_courses_preview.php', function(resp){
        if(resp.status === 'success'){
            let html = '';
            if(resp.items.length === 0){
                html = '<div class="empty">You have not enrolled in any course yet.</div>';
            } else {
                resp.items.forEach(item=>{
                    html += `<div class="item">
                        <img src="<?= BASE_URL ?>/${item.image}" alt="${item.title}">
                        <div class="item-info">
                            <div class="title">${item.title}</div>
                            <div class="price">Enrolled</div>
                        </div>
                        <a href="<?= BASE_URL ?>/product.php?id=${item.id}" class="btn btn-sm btn-primary">View</a>
                    </div>`;
                });
            }
            $('.dropdown-mycourses').html('<span class="close-btn">&times;</span>'+html);
            $('.dropdown-mycourses .close-btn').click(()=>{ $('.dropdown-mycourses').removeClass('active'); overlay.removeClass('active'); });
        }
    });
    <?php endif; ?>
});
